...
const OfferCard: React.FC<Offer> = ({ benefits, cost, description, id, offerType, title, selected }) => {
    ...
    const SELECTED_STYLE = "ring-2 ring-offset-2 ring-blackCP";
    ...
    const renderBenefits = () => {
        return Array.from(benefits.entries()).map(([benefit, isAvailable]) => (
            <li key={benefit} className="flex items-center gap-1">
                <svg
                    ...
                    strokeWidth="1.5"
                    stroke="currentColor"
                    className={`size-5 ${isAvailable ? 'text-indigo-700' : 'text-red-700'}`} // Blue if true, red if false
                >
                    {isAvailable ? (
